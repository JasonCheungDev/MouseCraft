...
void main() 
{
	...
	vec3 nrm = normalize(texture(u_NrmTex, f_Uv).rgb); 
    ...
    // float attenutation = brightness / (1 + exp(4 * distance / u_LightRange - 4));
   
    // diffuse (remove this for HARD lighting)
	vec3 lightDir = normalize(u_LightPos - pos);            // reversed to prevent negative signs 
    float diffuseStrength = max(0.0, dot(nrm, lightDir));    

 	vec3 diffuse = (u_LightColor * attenutation * diffuseStrength);
